...
describe('user reducer', () => {
  describe('should auth user', () => {
    it('should set initial state', () => {
      expect(user(undefined, {}).toJSON()).toEqual(fromJS({
        isFetching: true,
        error: '',
        isAuthed: false,
        authedId: '',
    ...
    it('should changed authed to true, and place user id on authedId', () => {
      const id = 1234;
      expect(user(undefined, actions.authUser(id)).toJSON()).toEqual(fromJS({
        isFetching: true,
        error: '',
        isAuthed: true,
        authedId: id,
  ...
  describe('on error', () => {
    it('should set error message if fetch failes', () => {
      ...
      expect(user(undefined, actions.fetchingUserFailure(err)).toJSON()).toEqual(fromJS({
        isFetching: false,
        error: err,
        isAuthed: false,
        authedId: '',
